...
- `RecordWriterAndCompatibilityTests.cs` covers explicit materialized-record metadata, explicit writer termination, independent content/separator writes, caller-owned output streams, and the preserved behavior of `Icod.CommandFramework.IO.DelimitedByteRecordReader`.
