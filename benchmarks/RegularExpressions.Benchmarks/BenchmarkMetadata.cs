...
internal static class BenchmarkMetadata {
	internal static void WriteRequestedMetadata() {
		var path = Environment.GetEnvironmentVariable(
			"ICOD_REGEX_BENCHMARK_METADATA_PATH"
		);
	...
	internal static void Write( string path ) {
		...
		var metadata = new {
			...
			Source = Environment.GetEnvironmentVariable( "ICOD_REGEX_BENCHMARK_SOURCE" ) ?? "Unspecified",
			Label = Environment.GetEnvironmentVariable( "ICOD_REGEX_BENCHMARK_LABEL" ) ?? "Unspecified",
			Commit = Environment.GetEnvironmentVariable( "ICOD_REGEX_BENCHMARK_COMMIT" ) ?? TryRunProcess( "git", "rev-parse HEAD" ),
			AssemblyVersion = typeof( GnuBasicRegularExpressionProvider ).Assembly.GetName().Version?.ToString(),
			...
			BenchmarkFramework = "BenchmarkDotNet 0.15.8"
		};
	...
	private static string? TryGetCpuModel() {
		...
		if ( OperatingSystem.IsMacOS() ) {
			return TryRunProcess( "sysctl", "-n machdep.cpu.brand_string" );
		}
