...
namespace Icod.CommandFramework.Platform;
...
public readonly record struct SelinuxExecutionResult(
	int ExitCode,
	int ErrorNumber,
	string? Diagnostic
);

/// <summary>Injectable SELinux/native process boundary shared by SELinux-aware commands.</summary>
...
public sealed class NativeSelinuxPlatform : ISelinuxPlatform {
	...
	public bool TryGetFileContext(
		...
		errorNumber = 0;
		IntPtr pointer;
		var result = dereference
			? getfilecon( path, out pointer )
			: lgetfilecon( path, out pointer )
		;
		if ( result < 0 ) {
			errorNumber = Marshal.GetLastPInvokeError();
			return false;
	...
	/// <inheritdoc />
	public bool TryComputeProcessContext(
		string sourceContext,
		string executableContext,
		out string context,
		out int errorNumber
		...
		ArgumentNullException.ThrowIfNull( executableContext );
	...
	/// <inheritdoc />
	public SelinuxExecutionResult ExecuteWithContext(
		string context,
		IReadOnlyList<string> command,
		bool searchPath
	) {
		...
		if ( command.Count == 0 ) {
			return new SelinuxExecutionResult(
				125,
				0,
		...
		if ( setexeccon( context ) != 0 ) {
			...
			return new SelinuxExecutionResult(
				125,
				error,
				$"failed to set execution context: {DescribeError( error )}"
			);
		}
		...
		try {
			...
			try {
				...
				return new SelinuxExecutionResult(
					status,
					error,
					$"failed to execute '{command[0]}': {DescribeError( error )}"
				);
			} finally {
				Marshal.FreeHGlobal( argv );
			}
		} finally {
			// setexeccon affects the next exec only. If allocation or exec fails,
			// clear the pending context before returning to managed code.
			_ = setexeccon( null );
	...
	public string DescribeError( int errorNumber ) {
		...
		return pointer == IntPtr.Zero
			? $"native error {errorNumber}"
			: Marshal.PtrToStringUTF8( pointer ) ?? $"native error {errorNumber}"
		;
	}
...
#pragma warning disable SYSLIB1054 // LibraryImport would require changing the project unsafe/source-generation policy.
	...
	[DllImport( LibSelinux, SetLastError = true )]
	private static extern int getfilecon(
		[MarshalAs( UnmanagedType.LPUTF8Str )] string path,
		out IntPtr context
	);

	[DllImport( LibSelinux, SetLastError = true )]
	private static extern int lgetfilecon(
		[MarshalAs( UnmanagedType.LPUTF8Str )] string path,
		out IntPtr context
	);

	[DllImport( LibSelinux, SetLastError = true )]
	private static extern int setfilecon(
		[MarshalAs( UnmanagedType.LPUTF8Str )] string path,
		[MarshalAs( UnmanagedType.LPUTF8Str )] string context
	);

	[DllImport( LibSelinux, SetLastError = true )]
	private static extern int lsetfilecon(
		[MarshalAs( UnmanagedType.LPUTF8Str )] string path,
		[MarshalAs( UnmanagedType.LPUTF8Str )] string context
	);

	[DllImport( LibSelinux, SetLastError = true )]
	private static extern int security_check_context(
		[MarshalAs( UnmanagedType.LPUTF8Str )] string context
	);

	[DllImport( LibSelinux, SetLastError = true )]
	private static extern ushort string_to_security_class(
		[MarshalAs( UnmanagedType.LPUTF8Str )] string name
	);

	[DllImport( LibSelinux, SetLastError = true )]
	private static extern int security_compute_create(
	...
	[DllImport( LibSelinux, SetLastError = true )]
	private static extern int setexeccon(
		[MarshalAs( UnmanagedType.LPUTF8Str )] string? context
	);
