...
`ByteRecordReader` is the preferred materializing API because it keeps content and termination metadata separate. `Icod.CommandFramework.IO.DelimitedByteRecordReader` remains the compatibility API; it now delegates to `ByteRecordReader` and continues returning arrays that include a present separator.
